...
CLANG_NOINLINE
void RK4_advance(fType dt, timing_struct * restrict t, int NPts, fType CQ_POOL(3) * restrict dS,fType CQ_POOL(3) * restrict Stmp,fType CQ_POOL(3) * restrict K1,fType CQ_POOL(3) * restrict S,fType CQ_POOL(3) * restrict K, SWE_struct* restrict SWE){
  ...
  //fType *K1 = (fType*) cq_malloc(3,NPts * sizeof(fType));
  //fType CQ_POOL(3)* K1 = (fType CQ_POOL(3) *) cq_malloc(3,NPts * sizeof(fType));
  //fType CQ_POOL(3) * restrict dS;
  //fType CQ_POOL(3) * restrict K1;
  //fType CQ_POOL(3) * restrict K2;
  //fType CQ_POOL(3) * restrict K3;
  //fType CQ_POOL(3) * restrict K4;
  //fType CQ_POOL(3) * restrict Stmp;
 ...
 // local_timer.t_rhs +=  (getTime() - t_start);
  ...
  //t_start = getTime();
  for(int i=0; i<NPts; i++){
    Stmp[i] = S[i] + dt*K1[i];  // 2 flop/site
    dS[i] += 2.0*K1[i];         // 2 flop/site
  }
  //halo_update(Stmp);
  //local_timer.t_update +=  (getTime() - t_start);
  ...
  //t_start = getTime();
  for(int i=0; i<NPts; i++) {
    S[i] += (sixth*dt)*(dS[i]+K1[i]); //2 flop/site
  }
  //halo_update(S);
  //local_timer.t_update +=  (getTime() - t_start);
