...
uint64_t cq_canonicalize_pointer(void* p);
...
void   cq_abort();                      // Prevents LLVM from doing something 
                                        // undesirable
void   cq_halt();                       // Trapped in infinite loop

// number of nanoseconds expired since the first call to this function

uint64_t cq_timestamp();                // number of clock cycles on FPGA
extern int32_t cq_timescale;            // value set by cqdataflowbase to 
                                        // 1,024,000/<FPGA Clock MHz>
#ifdef CQ_OLD_TIMER
//extern "C" uint64_t cq_nstime() {
__always_inline uint64_t cq_nstime() {
    //returns the number of nanoseconds expired since the first call to this
    // function
    return (uint64_t) (get_timer() * 1e9);
}
#else
__always_inline uint64_t cq_nstime() 
{
   return ((cq_timestamp() * cq_timescale) >> 10);
}
#endif
...
// Override atoi because llvm inserts a stupid function call for this.
#define atoi(s)         ((int)strtol((s),NULL,10))
#define atol(s)         ((long)strtol((s),NULL,10))
...
public:
    ...
    // member var used to initiate static initialization of memory pools in .cpp
    static bool _static_init;
};

typedef std::map<int, MemPoolDef> MemPoolDefs;

bool initialize_memory_pools(const MemPoolDefs& defs);
